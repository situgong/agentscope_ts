 ...
 * box you are typing into cannot slide away under a long result list.
 */
export function ResourcePanel({ title, description, icon, search, children }: Props) {
	return (
		<div className="flex h-full flex-col">
			<div className="flex shrink-0 flex-col gap-y-4 px-6 pt-6">
			...
			{/* min-h-0 so the flex child may shrink below its content height —
			    ...
			    with no overflow it shows nothing. */}
			<div className="flex-1 min-h-0 overflow-y-auto scroll-fade px-6 pb-6 pt-4">
				{children}
			</div>
		</div>
	);
